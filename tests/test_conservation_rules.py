...
def test_ptdf_conservation(tmp_path):
    ...
    jaodata_and_nps = fetch_jao_data_basecase_nps_and_observed_nps(start, end)

    for zone in jaodata_and_nps.observedNPs.columns:
        if zone not in [
        ...
        assert_series_equal(recomputed_np, jaodata_and_nps.observedNPs.loc[:, zone.value], atol=2)
