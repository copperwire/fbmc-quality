...
def test_entsoe_data(tmp_path):
    ...
    from fbmc_quality.entsoe_data.fetch_entsoe_data import fetch_observed_entsoe_data_for_cnec, _get_cross_border_flow_from_api, convert_date_to_utc_pandas, lookup_entsoe_areas_from_bz, query_and_cache_data
    from fbmc_quality.entsoe_data.fetch_entsoe_data import BIDDINGZONE_CROSS_BORDER_NP_MAP
    ...
    for from_zone, to_zones in BIDDINGZONE_CROSS_BORDER_NP_MAP.items():
        for to_zone in to_zones:
            ...
            query_and_cache_data(convert_date_to_utc_pandas(from_time), convert_date_to_utc_pandas(to_time), from_area, to_area, engine)
